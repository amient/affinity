...
object AvroSerde {
  ...
  class AvroConf extends CfgStruct[AvroConf] {
    val Class = cls("schema.registry.class", classOf[AvroSerde], true)
      .doc("one of ConfluentSchemaRegistry, ZookeeperSchemaRegistry, LocalSchemaRegistry or MemorySchemaRegistry from the io.amient.affinity.avro package")
...
trait AvroSerde extends AbstractSerde[Any] with AvroSchemaRegistry {
  ...
  /**
    ...
    */
  override def prefix(cls: Class[_ <: Any], prefix: Object*): Array[Byte] = {
    ...
    prefix.zip(schema.getFields.asScala.take(prefix.length).map(_.schema.getFixedSize)).foreach {
      case (value: Array[Byte], fixedLen: Int) if (value.length == fixedLen) => output.write(value)
      ...
      case (value: UUID, 16) => output.write(ByteUtils.uuid(value))
    }
    output.toByteArray
  }
