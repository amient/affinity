...
object AffinityKafkaAvroDeserializer {
  def create[T: TypeTag](props: Map[String, AnyRef], isKey: Boolean): Deserializer[T] = {
    if (props.contains("schema.registry.url")) {
      new TypedKafkaAvroDeserializer(props, isKey)
    } else {
      //TODO implement ZkAvroSchemaProvider Deserializer
      throw new RuntimeException("schema.registry configuration is missing in the consumer properties")
    }
  }
}
class TypedKafkaAvroDeserializer[T: TypeTag](props: Map[String, AnyRef], isKey: Boolean) extends Deserializer[T] {
