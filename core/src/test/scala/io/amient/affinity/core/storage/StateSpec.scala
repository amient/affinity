...
case class ExpirableValue(data: String, val eventTimeMs: Long) extends AvroRecord[ExpirableValue] with EventTime
...
class StateSpec extends TestKit(ActorSystem.create("test", ConfigFactory.defaultReference()))
  ...
  it should "work without ttl" in {
    ...
    val state = new State[Long, ExpirableValue]("test-state", system, stateConfig)
    ...
    state(1L) should be (ExpirableValue("one", nowMs - 9000))
